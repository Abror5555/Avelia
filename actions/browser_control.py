...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".avelia_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".avelia_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using AVELIA profile")
                avelia = str(Path.home() / ".avelia_profiles" / "firefox_avelia")
                Path(avelia).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(avelia, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".avelia_profiles" / "safari")
        ...
        # automation). Fall back to a persistent AVELIA automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        avelia_profile = str(Path.home() / ".avelia_profiles" / self.browser_name)
        Path(avelia_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with AVELIA profile: {avelia_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(avelia_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with AVELIA profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "avelia_screenshot.png")
