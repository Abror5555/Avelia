"""
Drop-in AVELIA plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — AVELIA discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the AveliaUI instance — use player.write_log(f"AVELIA: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"AVELIA: {result_text}")
        except Exception:
            pass
    return result_text
