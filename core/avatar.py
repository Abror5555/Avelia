...
* **Lip-sync comes from the audio pipeline, not from the avatar.** `main.py`
  ...
  and AVELIA's own output. The avatar just consumes that number, so there is no
  second audio path to fall out of sync. The mouth only tracks the level while
  AVELIA is *speaking* — during listening the same level drives the aura, so the
...
# to tell speech from room tone. AVELIA's own voice arrives as a per-20 ms
...
class HoloAvatar:
    ...
    def _mouth_step(self, dt: float, amp: float, live: bool,
        ...
        """One increment of the jaw. Called once per viseme frame while AVELIA
    ...
    def step(self, dt: float, amp: float, speaking: bool = False,
        ...
        # changes when AVELIA starts or stops talking jumps the phase by
