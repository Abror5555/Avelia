...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are AVELIA, Abror's AI assistant. "
...
class AveliaLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                def _shutdown():
                    import time, os
                    # Enough time for the goodbye line to actually finish playing
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            ...
            if not avelia_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"Avelia: {full_out}")
                                if self._dashboard:
                                    asyncio.create_task(self._dashboard.broadcast({
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until AVELIA finishes speaking the answer
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        Two-phase briefing for instant perceived response:
          Phase 1 — immediate greeting (no tools, no fetch) → Avelia speaks in <2s
    ...
    async def _briefing_news_phase(self, lang: str) -> None:
        ...
        # 4.5 s gives Avelia enough time to actually finish speaking the greeting
        # (including saying the current time) before phase-2 news is injected.
        ...
        p2 = (
            "[BRIEFING] Call web_search with mode='news' and query='XAUUSD gold price Dollar Index DXY today market analysis' "
            "to find actual, current financial market data and analysis. Focus specifically on two main pairs: "
            "Gold vs US Dollar (XAUUSD) and the US Dollar Index (DXY). "
            "After the search, give a full spoken explanation of both: the current price/trend, key support and "
            "resistance levels, and what is driving the movement (economic data, Fed policy, geopolitical factors, etc). "
            f"Then mention that the rest of the market data is shown in full detail on the screen.{lang_str}"
        )
